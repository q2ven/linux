...
#ifdef CONFIG_NET_SPLIT_RTNL_LOCK
extern int rtnl_net_is_locked(struct net *net);
extern bool lockdep_rtnl_net_is_held(struct net *net);
#else
#define rtnl_net_is_locked(net) rtnl_is_locked()
#define lockdep_rtnl_net_is_held(net) lockdep_rtnl_is_held()
#endif
#else
#define rtnl_net_is_locked(net) rtnl_is_locked()
