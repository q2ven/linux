...
struct tcp_out_options {
	...
	u16 edo_hdr;		/* Extended Data Offset Header Length */
	u16 edo_seg;		/* Extended Data Offset Segment Length */
			      ...
			      struct tcp_out_options *opts)
{
	...
	if (OPTION_EDO_EXT_HDR & options) {
		if (OPTION_EDO_EXT_SEG & options) {
			*ptr++ = htonl((TCPOPT_EDO_EXTENSION << 24) |
				...
				struct tcp_md5sig_key **md5)
{
	...
	*md5 = NULL;
	...
	if (tp->edo) {
		...
		remaining -= TCPOLEN_EXP_EDO_BASE;
	}
