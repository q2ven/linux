...
#define OPTION_EDO_EXTENSION	BIT(13)
#define OPTION_NOPS		BIT(14)
			      ...
			      struct tcp_key *key)
{
	...
	if (OPTION_EDO_EXTENSION & options) {
		if (tp->edo == TCP_EDO_HDR) {
			*ptr++ = htonl((TCPOPT_NOP << 24) | (TCPOPT_NOP << 16) |
				       (TCPOPT_EXP_EDO << 8) |
					...
					struct tcp_key *key)
{
	...
	if (tp->edo) {
		opts->options |= OPTION_EDO_EXTENSION;
			      ...
			      int clone_it, gfp_t gfp_mask, u32 rcv_nxt)
{
	...
	if (tcp_header_size < MAX_TCP_HEADER_SPACE)
		*(((__be16 *)th) + 6) = htons(((tcp_header_size >> 2) << 12) |
					      tcb->tcp_flags);
	else
		*(((__be16 *)th) + 6) = htons(((MAX_TCP_HEADER_SPACE >> 2) << 12) |
					      tcb->tcp_flags);
