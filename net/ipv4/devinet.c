...
{
	ASSERT_RTNL();
	hlist_del_init_rcu(&ifa->hash_node);
}
...
struct in_ifaddr *inet_lookup_ifaddr_rcu(struct net *net, __be32 addr)
{
	...
	hlist_for_each_entry_rcu(ifa, &net->ipv4.inet_addr_lst[hash], hash_node)
...
static void check_lifetime(struct work_struct *work)
{
	...
	for (i = 0; i < IN4_ADDR_HSIZE; i++) {
		...
		rcu_read_lock();
		hlist_for_each_entry_rcu(ifa, head, hash_node) {
...
int devinet_ioctl(struct net *net, unsigned int cmd, struct ifreq *ifr)
{
	...
	struct in_ifaddr *ifa = NULL, *ifa_tmp;
	...
	if (in_dev) {
		if (tryaddrmatch) {
			...
			in_dev_for_each_ifa_rtnl(ifa_tmp, in_dev) {
				if (!strcmp(ifr->ifr_name, ifa_tmp->ifa_label) &&
				    sin_orig.sin_addr.s_addr ==	ifa_tmp->ifa_local) {
					ifa = ifa_tmp;
					break; /* found */
				}
			}
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	if (dev && !__in_dev_get_rtnl_net(dev_net(dev), dev))
		return -EAFNOSUPPORT;
