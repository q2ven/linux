				   ...
				   const unsigned char **ptr, int *left, int opsize)
{
	...
	*thp = th = tcp_hdr(skb);
	...
	/* opcode and opsize are parsed but not minues from *left. */
	parsed = th->doff * 4 - sizeof(struct tcphdr) - (*left - 2);
 ...
 */
int tcp_parse_options(const struct net *net, struct sk_buff *skb,
		      ...
		      struct tcp_fastopen_cookie *foc, bool parse_edo_ext)
{
	...
	while (length > 0) {
		...
		default:
			...
			case TCPOPT_EXP:
				if (opsize >= TCPOLEN_EXP_EDO_BASE &&
				    get_unaligned_be16(ptr) == TCPOPT_EDO_MAGIC) {
					int ret;

					if (opsize == TCPOLEN_EXP_EDO_BASE && th->syn && !estab) {
						opt_rx->edo_ok = 1;
						break;
					}
				   ...
				   struct tcp_sock *tp)
{
	...
	if (tcp_parse_options(net, skb, &tp->rx_opt, 1, NULL, tp->edo)) {
		kfree_skb(skb);
		return -EINVAL;
	}
				  ...
				  bool parse_edo_ext)
{
	...
	while (length >= TCPOLEN_EDO_EXT_HDR) {
		int opcode = *ptr++;
		int opsize;
		...
		default:
			...
			} else if (opcode == TCPOPT_EXP && parse_edo_ext && !th->syn &&
 ...
 */
static bool tcp_validate_incoming(struct sock *sk, struct sk_buff *skb, int syn_inerr)
{
	struct tcp_sock *tp = tcp_sk(sk);
...
slow_path:
	...
	if (!tcp_validate_incoming(sk, skb, 1))
		return;
...
int tcp_rcv_state_process(struct sock *sk, struct sk_buff *skb)
{
	...
	if (!tcp_validate_incoming(sk, skb, 0))
		return 0;
			     ...
			     struct sk_buff *skb, const struct sock *sk)
{
	...
	tcp_rsk(req)->edo = rx_opt->edo_ok & tcp_sk(sk)->edo;
	tcp_rsk(req)->edo_seg = tcp_sk(sk)->edo_seg;
