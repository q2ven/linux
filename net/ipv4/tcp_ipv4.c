...
#ifdef CONFIG_TCP_MD5SIG
#define OPTION_BYTES (TCPOLEN_EDO_EXT_ALIGNED + sizeof(__be32) + TCPOLEN_MD5SIG_ALIGNED)
#else
#define OPTION_BYTES (TCPOLEN_EDO_EXT_ALIGNED + sizeof(__be32))
#endif
...
static void tcp_v4_send_reset(const struct sock *sk, struct sk_buff *skb)
{
	...
	/* Can't co-exist with TCPMD5, hence check offset */
	if (!key || edo) {
			    ...
			    int reply_flags, u8 tos, u32 txhash, u8 edo)
{
	...
	if (edo) {
		u16 hdr_len = sizeof(struct tcphdr) + TCPOLEN_EDO_EXT_ALIGNED;
