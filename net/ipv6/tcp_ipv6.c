...
static int tcp_v6_md5_hash_skb(char *md5_hash, const struct tcp_md5sig_key *key,
				 ...
				 u8 tclass, __be32 label, u32 priority, u32 txhash, u8 edo)
{
	...
	if (edo)
		tot_len += TCPOLEN_EDO_EXT_ALIGNED;
	...
	if (edo) {
		if (edo == TCP_EDO_HDR_SEG) {
			*topt++ = htonl((TCPOPT_EDO_EXTENSION << 24) |
...
static void tcp_v6_send_reset(const struct sock *sk, struct sk_buff *skb)
{
	...
	if (sk && sk_fullsock(sk)) {
		...
		genhash = tcp_v6_md5_hash_skb(newhash, key, NULL, skb, th->doff << 2);
