		     ...
		     u8 type, u8 code, int offset, __be32 info)
{
	...
	if (!sk || READ_ONCE(udp_sk(sk)->encap_type)) {
		...
		if (static_branch_unlikely(&udpv6_encap_needed_key)) {
			sk = __udp6_lib_err_encap(net, hdr, offset, uh, sk,
						  skb, opt, type, code, info);
			if (!sk)
				return 0;
		} else
			sk = ERR_PTR(-ENOENT);
				    ...
				    const struct in6_addr *daddr)
{
	...
	unsigned int hash2 = 0, hash2_any = 0;
...
int udpv6_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	DECLARE_SOCKADDR(struct sockaddr_in6 *, sin6, msg->msg_name);
	struct ipv6_txoptions *opt = NULL, *opt_to_free = NULL;
...
do_append_data:
	...
	err = ip6_append_data(sk, ip_generic_getfrag, msg, ulen,
			      sizeof(struct udphdr), &ipc6, fl6, dst_rt6_info(dst),
