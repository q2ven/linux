			     ...
			     int size, int value_regno, int insn_idx)
{
	...
	is_ptr_spill = value_regno >= 0 &&
	    is_spillable_regtype(state->regs[value_regno].type);

	if (!env->allow_ptr_leaks) {
		bool sanitize = is_ptr_spill;
                         ...
                         u32 next_idx, u32 curr_idx)
{
	...
	if (branch && insn) {
		...
		if (BPF_SRC(insn->code) == BPF_K) {
			...
			mark_reg_unknown(regs, insn->dst_reg);
			mark_reg_unknown(regs, insn->src_reg);
		}
	}
	return branch;
}
			     ...
			     struct bpf_insn *insn, int *insn_idx)
{
	...
	if (BPF_SRC(insn->code) == BPF_K &&
		...
		if (opcode == BPF_JEQ) {
			/* Only follow the goto, ignore fall-through. If
			 * needed, push the fall-through branch for simulation
			...
			*insn_idx += insn->off;
			...
			/* Only follow the fall-through branch, since that's
			 * where the program will go. If needed, push the goto
			 * branch for simulation under speculative execution.
			 */
			if (!env->allow_ptr_leaks &&
				...
				u32 off, u32 cnt)
{
	...
	for (i = off; i < off + cnt - 1; i++)
		/* Expand insni[off]'s seen flag to the patched range. */
